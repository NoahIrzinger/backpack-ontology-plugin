...
description: >
  ...
  "refine", "graph documentation", or "graph to KB".
metadata:
  version: "0.1.0"
---
...
step in the knowledge pipeline: `backpack-mine` extracts raw ore, `backpack-refine`
processes it into readable, reusable prose documents.

Mining extracts raw relational knowledge. Refining turns it into documents that humans
...
`Refining <graph> (<N> nodes, <E> edges) into <K> KB documents.`
...
| Research a topic end-to-end | `backpack-mine` → `backpack-refine` |
| Refresh docs after expanding | `backpack-mine` (continue) → `backpack-refine` (re-run) |
| Understand an unfamiliar graph | `backpack-refine` → read the Overview first |
| Hand off a graph to a collaborator | `backpack-refine` → share the KB docs alongside the graph |
| Deep analysis | `backpack-mine` → `backpack-refine` → `backpack-distill` → use signals as input to decisions |

## End of Run

After generating all documents, suggest the next step in the pipeline:

`Documents saved. Want me to distill the graph for signals? (/distill)`

This completes the **mine → refine → distill** pipeline. Each step is independently
...
4. Plan: "Refining spanish-wines (82 nodes, 140 edges) into 6 KB documents."
