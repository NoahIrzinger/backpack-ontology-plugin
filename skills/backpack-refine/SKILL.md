---
name: backpack-refine
description: >
  This skill should be used when the user asks to "refine a graph",
  ...
  knowledge. Also use when the user mentions "refine", "signals",
...
Refine your knowledge down to the signals that matter. The final step in the knowledge
pipeline: `backpack-mine` digs up raw ore, `backpack-extract` pulls out the usable
material, `backpack-refine` concentrates it down to the purest actionable insights.

Refining runs structural detectors across all graphs and KB documents, then enriches
...
- The backpack is empty — there's nothing to refine
...
`Refining signals across <N> graphs and <M> KB documents.`
...
| Periodic health check | `backpack-refine` alone (re-run on existing graphs) |
| After mining more content | `backpack-refine` to catch new structural issues |
| Deep analysis | Full pipeline → use critical/high signals as input to decisions |
| Graph quality maintenance | `backpack-refine` → fix flagged issues → `backpack-refine` again to verify |

Each step is independently useful. A user can refine without mining or extracting — any
graph with content can produce signals.

## Re-running

Signals are regenerable. Re-running refine on the same backpack:
...
Run refine periodically on growing graphs to catch drift, new gaps, or emerging patterns.
...
**Running refine on an empty backpack.** If there are no graphs or all graphs are empty,
...
Summary: "Refined 8 signals across 3 graphs. 1 critical: unaddressed inflammation
